...
{
	if (numParameters > 0) {
		m_rootParameters.reset(new RootParameter[numParameters]);
	}
	else {
...
void RootSignature::InitRootParameters()
{
	...
	m_rootParameters[0].InitAsConstantBuffer(0, D3D12_SHADER_VISIBILITY_VERTEX);
	m_rootParameters[1].InitAsConstantBuffer(1, D3D12_SHADER_VISIBILITY_VERTEX);
	m_rootParameters[2].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 1);
...
void RootSignature::Finalize() 
{
	...
	*/
	...
	RootDesc.pParameters = (const D3D12_ROOT_PARAMETER*)m_rootParameters.get();
	RootDesc.NumStaticSamplers = m_numStaticSamplers;
	RootDesc.pStaticSamplers = (const D3D12_STATIC_SAMPLER_DESC*)m_staticSamplers.get();
	...
	HRESULT hr;
	ASSERT_SUCCEEDED(hr = D3D12SerializeRootSignature(&RootDesc, D3D_ROOT_SIGNATURE_VERSION_1_0, signature.GetAddressOf(), error.GetAddressOf()));
